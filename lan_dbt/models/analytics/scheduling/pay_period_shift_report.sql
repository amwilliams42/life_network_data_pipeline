...
with
    ...
    -- Get all pay periods for each source database
    pay_periods as (
        select
            source_database,
            pay_period_id,
    ...
    schedule_raw as (
        select
            s.*,
            pp.pay_period_id as report_pay_period_id,
        ...
        inner join pay_periods pp
            ...
            and s.date_line >= pp.pay_period_start
        ...
        where
            ...
            not (s.assignment_status = 'OPEN' and s.is_training = true)
